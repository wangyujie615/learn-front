...
- 策略一：忽略节点跨层级操作场景，提升比对效率**（基于树进行对比)**。这一策略需要进行树比对，**即对树进行分层比较**。树比对的处理手法是非常“暴力”的，即两棵树只对同一层次的节点进行比较，**如果发现节点已经不存在了，则该节点及其子节点会被完全删除掉，不会用于进一步的比较，这就提升了比对效率**。**(分层对比)**

- 策略二：如果组件的class一致，则默认为相似的树结构，否则默认为不同的树结构**（基于组件进行对比**）。
    在组件比对的过程中：
    
    - 如果组件是同一类型，则进行属性的更新，通过Hooks状态判断是否需更新；

    - 如果不是则直接放入补丁中。销毁旧节点及其子树，创建新节点及子树，并插入 DOM。
...
- 策略三：同一层级的子节点，可以通过标记 key 的方式进行列表对比**（基于节点进行对比）**。元素比对主要发生在同层级中，通过标记节点操作生成补丁。节点操作包含了插入、移动、删除等。其中节点重新排序同时涉及插入、移动、删除三个操作，所以效率消耗最大，此时策略三起到了至关重要的作用。**通过标记 key 的方式，React 可以直接移动 DOM 节点，降低内耗。**
    ...
    **匹配规则**：
    ...
    - 遍历新列表，通过`key`查找旧节点；若找到则复用，未找到则创建
    - 记录旧节点中为被复用的节点，执行删除
    - 执行上述的操作后：节点的顺序变动就变成移动，而不是进行重建。
...
- **静态节点标记**：
   模板中的静态节点（无响应式绑定）会被编译为常量，跳过Diff

  ```html
  html 体验AI代码助手 代码解读复制代码<!-- 编译前 -->
  <div>Hello Vue</div>
...
- 手动控制更新：需通过 `React.memo、React.memo、useMemo`避免无效渲染
...
| **优化方向** | **运行时优化**（Fiber调度） | **编译时优化**（模板静态分析） |
...
`react` 的所有事件并没有绑定到具体的`dom`节点上而是**绑定在了`document`上**，然后**由统一的事件处理程序来处理，同时也是基于浏览器的事件机制（冒泡）**，所有节点的事件都会在 `document`上触发。
