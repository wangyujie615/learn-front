...
- 动态路由： /blog/[:id]
- 路由跳转：<Link>组件
- 拦截路由：拦截路由允许你在当前布局中加载来自应用程序其他部分的路由。约定来定义拦截路由`(..)`，它类似于相对路径约定，`../`但针对的是路由段。
  - `(.)`匹配**同一级别的段**
  - `(..)`匹配**上一级的段**
  - `(..)(..)`匹配**上两级的段**
  - `(...)`匹配**根** `app`目录中的段

- 平行路由：并行路由允许您在同一布局中同时或有条件地渲染一个或多个页面。它们对于应用中高度动态的部分非常有用，例如仪表板和社交网站上的动态。**并行路由使用命名插槽**创建。插槽按照约定定义`@folder`。插槽作为 props 传递给共享父布局。
- 路由组：路线组是一种文件夹约定，可让您按类别或团队组织路线。(名称)表示路由组
...
Next.js 使用文件系统路由，其中**文件夹**用于创建嵌套路由。每个文件夹代表一个映射到**URL段的**路由段。
